...
bool bigger(int pos, string &x, int k1, int k2) {
    //cout << "query: " << pos << endl;
    for(int i = k1; i < k2; i++) {
        int j = i-k1;
...
bool query(string x, int k1, int k2) {
    int l = 0;
    int r = s.size()-1;
    int ans = 0;
...
int query2(string &x, int k) {
    ...
    while(l <= r) {
        int mid = (l+r)>>1;
        if(query(x,k,k+mid)) ans = mid, l = mid+1;
        else r = mid-1;
    }
