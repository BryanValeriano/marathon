...
ll solve(int at, int op) {
    ...
    if(!vis[at]) return dp[at][op] = 1LL + min( {solve(at+1,0) + abs(best[at][0] - pos), solve(at+1, 1) + abs(best[at][1] - pos),
            solve(at+1, 2) + abs(best[at][2] - pos), solve(at+1,3) + abs(best[at][3] - pos) });
    if(at == last) return dp[at][op] = min(travel(pos,ponta[at][0], ponta[at][1]), travel(pos, ponta[at][1], ponta[at][0]));

    ll esq1 = travel(pos, ponta[at][0], ponta[at][1]) + abs(best[at][2] - ponta[at][1]);
    ll esq2 = travel(pos, ponta[at][0], ponta[at][1]) + abs(best[at][3] - ponta[at][1]);
    ll dir1 = travel(pos, ponta[at][1], ponta[at][0]) + abs(best[at][0] - ponta[at][0]);
    ll dir2 = travel(pos, ponta[at][1], ponta[at][0]) + abs(best[at][1] - ponta[at][0]);

    ans = solve(at+1, 0) + dir1;
    ans = min(ans, solve(at+1, 1) + dir2);
    ans = min(ans, solve(at+1, 2) + esq1);
    ans = min(ans, solve(at+1, 3) + esq2);
